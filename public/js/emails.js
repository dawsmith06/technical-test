...
new Vue({
  ...
  methods: {
    ...
    getEmails: function getEmails() {
      ...
      this.showLoader(600);
      axios.get("emails-list".concat(this.request.toString()), {
        params: {
          page: this.page
        }
    ...
    store: function store() {
      ...
      this.showLoader();
      axios.post("emails", this.email).then(function (response) {
        if (response.data.success) {
          _this2.hideModal("#emails-form");
          ...
          _this2.alert("Email stored", " ", "success", 2000);

          _this2.getEmails();
        }
      })["catch"](function (error) {
        _this2.alert("Something went wrong", "Your changes could not be saved", "error", false);
      })["finally"](function () {
        _this2.hideLoader();
      });
    },
    ...
    destroy: function destroy(user) {
      ...
      axios["delete"]("users/" + user.id).then(function (response) {
        if (response.data.success) {
          _this3.alert("User deleted", " ", "success", 4000);
     ...
     * @param {object} user
     */
    confirmDestroy: function confirmDestroy(user) {
      var _this4 = this;

      swal({
        title: "Are you Sure?",
        text: "Do you want to delete ".concat(user.email, " user?\n"),
        icon: "warning",
        buttons: true,
        dangerMode: true
      }).then(function (willDelete) {
        if (willDelete) _this4.destroy(user);
      });
    },
     ...
     * @param {Object} user
     */
    onChange: function onChange(field, user) {
    ...
    orderBy: function orderBy(column) {
      var orderMode = this.order.mode == "ASC" ? "DESC" : "ASC";
  ...
  computed: {
    changes: function changes() {
      return this.emails.filter(function (user) {
        return user.changed;
      });
    },
    ...
    maxDate: function maxDate() {
      return "".concat(this.actualYear - 18, "-").concat(this.actualMonth, "-").concat(this.actualDay);
    },
    actualYear: function actualYear() {
      return new Date().getFullYear();
    },
    actualMonth: function actualMonth() {
